<script lang="ts">
  ...
  const highlightNodePartial = function (
    textNode,
    start,
    end,
    pageHighlights,
  ...
  const makeHighlight = function (isHighlight: boolean) {
    if (window.getSelection().anchorNode) {
      ...
      if (
        ancestorNode.nodeType === TEXT_NODE &&
        ancestorNode.parentNode.tagName === "SPAN" &&
        !ancestorNode.parentNode.hasAttribute("tree-id")
      ) {
        ancestorNodeId = nodeIdSpan(startNode);
      } else {
        while (
          ancestorNode.tagName === "SPAN" &&
          !ancestorNode.hasAttribute("tree-id")
        ) {
      ...
      if (
        startNode.parentElement.tagName === "SPAN" &&
        !startNode.parentElement.hasAttribute("tree-id")
      ) {
        startNodeOffset = convertSelectionWithoutSpan(
          startNode,
          range.startOffset
        );
      ...
      if (
        endNode.parentElement.tagName === "SPAN" &&
        !endNode.parentElement.hasAttribute("tree-id")
      ) {
